...
interface ActivityWithGroups extends Partial<Activity> {
  groups?: ({
    members?: ({
      groupMember?: {
        id?: string;
...
interface ActivitySettlementProps {
  activity: ActivityWithGroups;
}

export function ActivitySettlement({ activity }: ActivitySettlementProps) {
  const [expandedParticipants, setExpandedParticipants] = useState<Set<string>>(new Set());
  const [paymentAmount, setPaymentAmount] = useState<{ [key: string]: string }>({});
  const [isSubmitting, setIsSubmitting] = useState<{ [key: string]: boolean }>({});

  // 參與者
  const participants = (activity.groups ?? []).flatMap(group => 
  ...
  // 總支出
  const totalExpense = (activity.transactions ?? []).reduce((sum, t) => sum + (t.amount ?? 0), 0) || 0;
  // 每人應付
  const perPersonAmount = participants.length ? totalExpense / participants.length : 0;

  // 計算每個參與者的已付金額
  const calculatePaidAmount = (participantId: string) => {
    const payments = (activity.transactions ?? [])
      .filter(t => t.groupMember?.id === participantId && t.type === 'EXPENSE')
      .reduce((sum, t) => sum + (t.amount ?? 0), 0);
    const incomes = (activity.transactions ?? [])
          ...
          <div className="hidden sm:grid grid-cols-3 gap-4">
            ...
            <div className="space-y-2">
              ...
              <p className="text-2xl font-bold text-green-500">
                {(activity.transactions ?? []).filter(t => t.type === 'INCOME').reduce((sum, t) => sum + (t.amount ?? 0), 0).toFixed(2)}
              </p>
            </div>
            <div className="space-y-2">
              <p className="text-sm text-muted-foreground">淨支出</p>
              <p className="text-2xl font-bold">
                {(totalExpense - (activity.transactions ?? []).filter(t => t.type === 'INCOME').reduce((sum, t) => sum + (t.amount ?? 0), 0)).toFixed(2)}
              </p>
            </div>
          </div>
        ...
        <CardContent>
                ...
                {(activity.transactions ?? [])
                  ...
                  .map((transaction) => (
                    <TableRow key={transaction.id ?? Math.random()}>
                      ...
                      <TableCell>
                        <span className={`font-medium ${transaction.type === 'EXPENSE' ? 'text-red-500' : 'text-green-500'}`}>{transaction.type === 'EXPENSE' ? '支出' : '收入'}</span>
                      </TableCell>
                      <TableCell>{transaction.description || '無描述'}</TableCell>
                      <TableCell>
                        <span className={`font-medium ${transaction.type === 'EXPENSE' ? 'text-red-500' : 'text-green-500'}`}>{transaction.type === 'EXPENSE' ? '-' : '+'}${(transaction.amount ?? 0).toFixed(2)}</span>
                      </TableCell>
                      <TableCell>{transaction.groupMember?.user?.name || transaction.groupMember?.name || '無'}</TableCell>
          ...
          <div className="flex flex-col gap-3 sm:hidden">
            {(activity.transactions ?? [])
              ...
              .map((transaction) => (
                <div key={transaction.id ?? Math.random()} className="rounded-lg bg-gray-50 p-3 flex flex-col gap-1">
                  <div className="flex items-center gap-2">
                    <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${transaction.type === 'EXPENSE' ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800'}`}>{transaction.type === 'EXPENSE' ? '支出' : '收入'}</span>
                    <span className="text-xs text-muted-foreground">{transaction.date ? new Date(transaction.date).toLocaleDateString() : ''}</span>
                  </div>
                  <div className="text-base font-medium">{transaction.description || '無描述'}</div>
                  <div className="flex items-center gap-2">
                    <span className={`font-medium ${transaction.type === 'EXPENSE' ? 'text-red-500' : 'text-green-500'}`}>{transaction.type === 'EXPENSE' ? '-' : '+'}${(transaction.amount ?? 0).toFixed(2)}</span>
                    <span className="text-xs text-muted-foreground">{transaction.groupMember?.user?.name || transaction.groupMember?.name || '無'}</span>
            ...
            <Table className="w-full">
              ...
              <TableBody>
                {participants.map((participant) => {
                  ...
                  const participantTransactions = (activity.transactions ?? [])
                    .filter(t => t.groupMember?.id === participant.id)
                    .sort((a, b) => new Date(b.date ?? '').getTime() - new Date(a.date ?? '').getTime());
                  return (
                    <Fragment key={participant.id ?? Math.random()}>
                      <TableRow>
                        <TableCell>
                          <Button
                            variant="ghost"
                        ...
                        <TableCell>
                          <span className={`font-medium ${balance > 0 ? 'text-red-500' : balance < 0 ? 'text-green-500' : ''}`}>${Math.abs(balance).toFixed(2)}</span>
                        </TableCell>
                        <TableCell>
                          <span className={`px-2 py-1 rounded-full text-xs ${paymentStatus === '已結清' ? 'bg-green-100 text-green-800' : paymentStatus === '待付款' ? 'bg-red-100 text-red-800' : 'bg-yellow-100 text-yellow-800'}`}>{paymentStatus}</span>
                        </TableCell>
                        <TableCell>
                      ...
                      {isExpanded && participantTransactions.map((transaction) => (
                        <TableRow key={transaction.id ?? Math.random()} className="bg-muted/50">
                          <TableCell></TableCell>
                          <TableCell colSpan={6}>
          ...
          <div className="flex flex-col gap-4 sm:hidden">
            {participants.map((participant) => {
              ...
              const participantTransactions = (activity.transactions ?? [])
                ...
                .sort((a, b) => new Date(b.date ?? '').getTime() - new Date(a.date ?? '').getTime());
              return (
                <div key={participant.id ?? Math.random()} className="rounded-lg shadow bg-white p-4">
                  ...
                  <div className="flex flex-wrap gap-2 mt-2 text-sm">
                    ...
                    <div>狀態：<span className={`px-2 py-1 rounded-full text-xs ${paymentStatus === '已結清' ? 'bg-green-100 text-green-800' : paymentStatus === '待付款' ? 'bg-red-100 text-red-800' : 'bg-yellow-100 text-yellow-800'}`}>{paymentStatus}</span></div>
                  </div>
                    ...
                    <div className="mt-4 border-t pt-3">
                      ...
                      <div className="flex flex-col gap-2">
                        ...
                        {participantTransactions.map((transaction) => (
                          <div key={transaction.id ?? Math.random()} className="flex justify-between items-center p-2 rounded bg-gray-50">
                            <div>
                              <div className="flex items-center gap-2">
                                <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${transaction.type === 'EXPENSE' ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800'}`}>{transaction.type === 'EXPENSE' ? '支出' : '收入'}</span>
                                <span className="font-medium">{transaction.description || '無描述'}</span>
                              </div>
                              <div className="text-xs text-muted-foreground mt-1">{transaction.date ? new Date(transaction.date).toLocaleDateString() : ''} • {transaction.category?.name ?? ''}</div>
                            </div>
                            <span className={`font-medium ${transaction.type === 'EXPENSE' ? 'text-red-500' : 'text-green-500'}`}>{transaction.type === 'EXPENSE' ? '-' : '+'}${(transaction.amount ?? 0).toFixed(2)}</span>
                          </div>
                        ))}
